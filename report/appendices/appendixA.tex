...
\texttt{r\_frame\_rate} because the latter is a base rate FFmpeg infers from the timing grid and
need not describe the frames that are actually present.

A screen recording taken from the project's own test material shows how far apart the two can
be. The file reports \texttt{r\_frame\_rate} as 60000/1001, that is 59.94~fps, while
\texttt{avg\_frame\_rate} reports 28.725~fps; counting frames confirms the second, at 287 frames
in ten seconds. Every interval between consecutive frames turns out to be an integer multiple of
1/144~second --- four, five or six ticks, giving instantaneous rates of 36, 28.8 and 24~fps ---
so the real timing grid is the 144~Hz display, and neither the 59.94 that
\texttt{r\_frame\_rate} advertises nor the 30 the source game was capped at. The cap cannot be
met exactly, because 144 is not divisible by 30 and a frame would have to occupy 4.8 ticks.

The consequence for the group of pictures is direct. Taking
\texttt{avg\_frame\_rate} gives 172 frames, or 5.988~seconds, which fits inside the segment.
Taking \texttt{r\_frame\_rate} would give 359 frames, or 12.5~seconds, more than twice the
segment length. A fixed 30~fps assumption would give 180 frames, or 6.27~seconds, which also
overruns. This is also why \texttt{-force\_key\_frames} is retained alongside \texttt{-g}: when
the source rate varies between 24 and 36~fps, an interval counted in frames cannot correspond to
a fixed interval in time, and only the time-based expression can.

Once the rate is known the group of pictures is \texttt{floor(fps} $\times$ \texttt{6)} frames,
floored rather than rounded so that it never exceeds the segment length. The same count of 180
frames illustrates why the rate has to be right before the arithmetic can be: it lasts
6.27~seconds at the 28.725~fps just measured, and 6.006~seconds at 29.97~fps, where it arises
instead from rounding 179.82 upward. Both overrun a six-second segment, and in both cases the
boundary drifts past the point where a keyframe exists. The value shown below, 144, is what a
24~fps source produces.
...
\ref{tab:qoe-multisource}    & \path{scripts/qoe/compare.js}, driven by \path{docs/results/qoe-sources.json} & \path{qoe-playback-<source>-<profile>.json}, \path{qoe-comparison.json} \\
\ref{tab:qoe-dpr}            & \path{scripts/qoe/collect.js} with \texttt{--dpr} & \path{qoe-playback-<source>-unthrottled.json}, \path{qoe-playback-<source>-unthrottled-dpr2.json} \\
