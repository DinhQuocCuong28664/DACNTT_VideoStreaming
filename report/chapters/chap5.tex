...
that lists the available bandwidths, codecs and resolutions. Those bandwidths and codec strings
are derived from the encoded output rather than from the encoder settings: the bandwidth is the
peak segment bit rate measured from the segments themselves, which
Section~\ref{sec:qoe-playback} examines in detail, and the codec string is read out of each
stream's sequence parameter set, since the constraint-set byte that RFC~6381 requires appears in
neither the profile name nor the level number reported by a probe. A thumbnail is extracted in a
separate pass.
...
\texttt{403 Forbidden}, whatever the video's visibility. The function building the resource
...
Both of the alarms defined at that point watched the transcoding pipeline, the dead letter queue
and the queue depth, and neither watched the backend API, which runs as a
...
The automated test suite comprises 209 tests across fourteen suites, summarised in
...
\texttt{relatedVideos}      & 10 & That the recommendation endpoint reuses the same authorisation rule as the detail endpoint, and never surfaces a private or unlisted video \\
...
\path{transcoder/framerate} & 32 & That the group of pictures follows the real source frame rate, that a variable-frame-rate file cannot poison it, and that every rendition is forced to cut at the same instant \\
\path{transcoder/bandwidth} & 18 & That the declared bandwidth of each variant is the peak measured from the segments themselves rather than a configured target, rounded upward so it remains the upper bound RFC 8216 requires \\
\path{transcoder/codecs} & 32 & That the codec string declared for each variant is read from the encoded stream itself rather than derived from configuration, and is omitted rather than guessed when it cannot be read \\
\bottomrule
\end{tabularx}
\end{table}
