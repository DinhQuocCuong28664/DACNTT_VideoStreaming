...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/02-transcoding-sequence.png}
...
it originally was not is discussed in Section~\ref{sec:limitations}.
...
with the guarantees it would provide if it ran --- the same distinction that
Section~\ref{sec:limitations} draws about content protection.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/04-database-erd.png}
...
was not adopted here is scale rather than principle --- which is a different claim from the design
being correct in general, and is recorded as such.
...
Search is the exception, and it is worth stating plainly because the schema originally claimed
otherwise. The collection carried a text index on title and description, described as supporting
...
neither of which a text index does. At the scale of this project a scan is acceptable; recording
which of the two was actually chosen, and why, is what matters.
