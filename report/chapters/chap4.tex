...
rather than as a separate collection. This denormalisation is appropriate here because the
counts are always read together with the video and because a single user can appear at most
once in each array, which bounds the array size by the number of viewers rather than allowing
unbounded growth.
...
and sorts on the third. A text index on title and description supports the search feature.
