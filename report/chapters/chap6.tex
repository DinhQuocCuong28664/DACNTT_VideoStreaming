...
The Time-to-First-Frame measurement has since been repeated against the live distribution with
Signed Cookies attached, and is reported in Section~\ref{sec:cdn-measurement}; what remains open
there is narrower, namely that all samples were taken from a single client location. Since the
defect that measurement uncovered was itself a matter of where the client sits relative to the
edge, figures gathered from several geographic origins would characterise the delivery layer
considerably better than the present single vantage point can. The concurrency test re-run with
genuine video payloads (Section~\ref{sec:real-throughput-validation})
