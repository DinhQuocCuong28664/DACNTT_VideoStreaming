...
The \textbf{anonymous visitor} arrives without an account. This group needs to browse public
videos, search and filter by category, and watch content immediately, because requiring
registration before any content can be seen would deter adoption. The \textbf{registered user}
...
\textbf{system administrator}, in the context of this project the development team itself,
...
Chapter~\ref{chap:implementation} can report whether they were met.
...
NFR-03 & Reliability & A failed transcoding job must not silently disappear: it must be retried where retrying could succeed, and must raise an alert carrying the reason when it cannot. How this requirement was initially satisfied only in appearance is examined in Section~\ref{sec:limitations}. \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/05-use-case.png}
