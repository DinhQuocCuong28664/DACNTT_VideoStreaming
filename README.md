...
3. **Xem video bằng HLS & Trình phát tuỳ biến:** Hệ thống tích hợp thư viện `HLS.js` để phân tích tệp manifest `.m3u8`, tải các đoạn video `.ts` theo thời gian thực và phát nội dung trên trình duyệt. Trình phát được trang bị thanh tua thời gian (timeline scrubber) có tooltip hiển thị mốc thời gian và ảnh đại diện xem trước bám theo vị trí con trỏ chuột.
...
6. **Trang cá nhân – Channel Management:** Người dùng có thể quản lý danh sách video đã tải lên, theo dõi trạng thái xử lý (`UPLOADING`, `PROCESSING`, `READY`, `ERROR`), xem lượt xem, thao tác nhanh qua menu 3 chấm hiện đại: đổi trực tiếp trạng thái hiển thị (Public, Unlisted, Private), mở modal chỉnh sửa thông tin video (tiêu đề, mô tả, danh mục, tags, visibility với giao diện glassmorphic), hoặc xóa video (xóa sạch DB record và toàn bộ thư mục HLS trên S3).
7. **Gợi ý video liên quan (Related Videos) & Tính năng mở rộng:** Hệ thống gợi ý video liên quan (`GET /api/videos/:id/related`) hiển thị tại thanh bên (sidebar) của trang xem video, tự động đề xuất dựa trên tag và danh mục tương đồng; thuật toán khoá cứng bộ lọc bảo mật chỉ hiển thị video công khai đã sẵn sàng (`visibility: 'public'` và `status: 'READY'`) đồng thời đóng kín lỗ hổng existence oracle đối với video riêng tư/chưa liệt kê. Bên cạnh đó là các tính năng: Tìm kiếm & lọc video (Search & Filter với regex escaping an toàn), nút Tương tác Like/Dislike, Hệ thống Bình luận (Comments), Đa ngôn ngữ (i18n Tiếng Việt & Tiếng Anh), Phân loại Danh mục (Categories) và Tương thích Responsive Mobile.
...
> ℹ️ **Trạng thái triển khai thực tế (cập nhật 2026-09-09):** CloudFront Distribution cho video CDN đã deploy và đang chạy trên production. Tài khoản AWS chính (lưu S3/Batch/Lambda) vẫn bị chặn tự tạo CloudFront Distribution (`AccessDenied: Your account must be verified`), nên CloudFront được vận hành trên một AWS account phụ thông qua cơ chế Terraform multi-account provider — S3 Processed Bucket vẫn ở account chính và hoàn toàn riêng tư qua Origin Access Control. **Signed Cookies nay đã bật** (`enable_signed_urls = true`, Trusted Key Group gắn từ 2026-08-30): mọi request video không kèm cookie hợp lệ đều nhận HTTP 403 `MissingKey` ngay tại Edge Location. Cổng ký này phủ toàn bộ video kể cả video công khai; riêng `*/thumbnail.jpg` được cố ý miễn trừ để trang danh mục vẫn hiển thị được ảnh đại diện. Xem Chương 6 của báo cáo (`report/chapters/chap6.tex`) để biết phân tích đầy đủ.
