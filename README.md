...
8. **Kiểm duyệt nội dung (Content Moderation):** Ngay trong job chuyển mã, transcoder lấy mẫu khung hình bằng FFmpeg (5 giây/khung, tối đa 120 khung) và gửi tới Amazon Rekognition `DetectModerationLabels`. Video có nội dung khiêu dâm, bạo lực, gây sốc hoặc biểu tượng thù ghét với độ tin cậy ≥ 90% bị **tự động gỡ**; trường hợp lưng chừng (≥ 60%) bị ẩn và **đưa vào hàng rà soát**. Kết quả được ghi cùng một lệnh với `status: READY`, nên không có khoảnh khắc nào video đã công khai mà chưa được kiểm duyệt. Người xem có nút **Báo cáo** với 8 lý do; đội vận hành rà soát tại trang `/admin` (ảnh bìa làm mờ mặc định, nhãn Rekognition kèm mốc thời gian, danh sách báo cáo, nút Giữ lại / Gỡ video / Khôi phục kèm ghi chú gửi chủ video). Quyền admin chỉ cấp được bằng `node backend/scripts/set-role.js <email> admin`. Chi tiết thiết kế xem tại [`docs/CONTENT_MODERATION.md`](docs/CONTENT_MODERATION.md).
