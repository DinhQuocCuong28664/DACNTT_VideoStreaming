...
> ✅ **Cập nhật trạng thái (2026-09-09):** Toàn bộ thiết kế mô tả trong tài liệu này **đã được triển khai và đang chạy trên production**. Cảnh báo cũ ngày 2026-08-10 (tài khoản AWS bị `AccessDenied` khi tạo CloudFront, bucket còn bucket policy `Principal: "*"` thêm thủ công) đã được xử lý xong: CloudFront chạy trên một AWS account phụ qua cơ chế multi-account provider, bucket `dacntt-dev-processed-bucket` nay hoàn toàn riêng tư sau Origin Access Control, và `enable_signed_urls = true` với Trusted Key Group `dacntt-dev-signing-key-group` (public key `K2KJYTIG6SBJUI`) đã gắn vào distribution `E2CMTN7QBKADP3` từ 2026-08-30. Kiểm chứng ngày 2026-09-09: request tới `cdn.zelostech.site` không kèm cookie trả về HTTP 403 `MissingKey: Missing Key-Pair-Id query parameter or cookie value` với header `Server: CloudFront`.
